...
def start_green_agent(agent_name="terminal_bench_green_agent", host="localhost", port=9001):
    ...
    # Also update provider URL to use public URL
    if "provider" in agent_card_dict:
        agent_card_dict["provider"]["url"] = base_url
    ...
    # Add CORS middleware for AgentBeats compatibility
    ...
    async def get_agent_status(request):
        return JSONResponse({"status": "healthy"}, headers={"Access-Control-Allow-Origin": "*"})

    async def get_info(request):
        """AgentBeats controller info endpoint"""
        return HTMLResponse(f"""
        <html>
        <head><title>Agent Controller - Info Panel</title></head>
        <body style="font-family: sans-serif; padding: 20px; background: #f5f5f5;">
            <h1>Agent Controller - Info Panel</h1>
            <p><strong>Running Agent:</strong> 1/1</p>
            <p><strong>Agent Name:</strong> {agent_card.name}</p>
            <p><strong>Agent URL:</strong> {base_url}</p>
            <p><strong>Status:</strong> <span style="color: green;">RUNNING</span></p>
            <h2>Agent Card</h2>
            <pre style="background: white; padding: 10px; border-radius: 5px;">{json.dumps(agent_card.model_dump(mode='json', exclude_none=True), indent=2)}</pre>
        </body>
        </html>
        """)
    ...
    starlette_app.routes.append(
        Route("/.well-known/agent-card.json", get_agent_card_json, methods=["GET", "OPTIONS"])
    )
    starlette_app.routes.append(
        Route("/.well-known/agent-card", get_agent_card_json, methods=["GET", "OPTIONS"])
    )
    starlette_app.routes.append(
