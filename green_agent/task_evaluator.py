...
class TaskEvaluator:
    ...
    def _parse_pytest_output(self, output: str, exit_code: int) -> Dict[str, Any]:
        ...
        Falls back to regex parsing when all tests pass (no summary section).
        """
        import re
        ...
        try:
            ...
            # Terminal Bench parser fails when there's no "short test summary info"
            # This happens when ALL tests pass - pytest doesn't output that section
            self.logger.warning(f"Terminal Bench parser failed: {e}, using fallback regex")
            
            # Fallback: parse the summary line like "19 passed in 0.09s"
            # or "5 passed, 2 failed in 1.23s"
            ...
            test_results = {}  # No per-test details in fallback mode
            
            # Also check for "All tests passed" message from run-tests.sh
            if "All tests passed" in output and tests_run == 0:
                # Count PASSED lines in verbose output
                passed_lines = re.findall(r'PASSED', output)
        ...
        return {
            ...
            "test_results": {name: status.value for name, status in test_results.items()} if test_results else {},
            "raw_output": output
        }
