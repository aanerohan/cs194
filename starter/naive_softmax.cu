...
__global__ void naive_row_softmax(const float* __restrict__ x, float* __restrict__ y, int N, int D) {
...
extern "C" void launch_softmax_kernel(const float* x_d, float* y_d, int N, int D, cudaStream_t stream) {
    dim3 grid(N), block(1024);
    naive_row_softmax<<<grid, block, 0, stream>>>(x_d, y_d, N, D);
}
