...
class TerminalBenchWhiteAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        # Send response (metadata not supported in newer a2a-sdk versions)
...
def start_white_agent(agent_name="terminal_bench_white_agent", host="localhost", port=9002, model="gpt-4o"):
    print(f"Starting white agent with model: {model}...")
    
    # Set model in environment for executor to use
